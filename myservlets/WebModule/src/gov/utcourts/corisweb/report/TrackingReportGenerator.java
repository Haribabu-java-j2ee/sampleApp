...
public class TrackingReportGenerator extends ReportBaseHandler {

	private static Logger logger = Logger.getLogger(TrackingReportGenerator.class.getName());
	
	private TrackingReportSearchCriteria thisCriteria;
	public static final String REPORT_NAME = "Tracking Report";
	private static final StringArrayDescriptor COLUMN_HEADERS = new StringArrayDescriptor("Type", "Rev Date", "Case", "", "Name", "Set Date", "Clerk", "Judge");
	private static final float[] PDF_COLUMN_WIDTHS = new float[] {22, 10, 10, 5, 26, 11, 8, 8};
		
	public TrackingReportGenerator(ReportBaseSearchCriteria criteria) {
		super(criteria);
		thisCriteria = (TrackingReportSearchCriteria) criteria;
	}
	...
	protected void generateExcelRowData(Row row, TrackingBO result) throws Exception {
		...
		cell.setCellValue(TextUtil.print(result.get(TrackingTypeBO.DESCR)));
	 ...
	 * @return
	 */
	private PdfPTable generatePdfResultTable(List<TrackingBO> searchResults) throws Exception {
	 ...
	 * @param withoutImgOnly
	 */
	private void generatePdfReportContents(PdfPTable table, List<TrackingBO> searchResults) throws Exception {
		if (searchResults != null && searchResults.size() > 0) {
			for (TrackingBO result : searchResults) {
				try {
					// type
					table.addCell(getPdfContentCell(TextUtil.print(result.get(TrackingTypeBO.DESCR))));
