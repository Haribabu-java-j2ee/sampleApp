...
public class PageVariables {
	...
	public PageVariables getFromRequest(HttpServletRequest request) {
		return getFromRequest(request, null);
	}
	
	public PageVariables getFromRequest(HttpServletRequest request, List<String> specificKeys) {
		...
		// RETRIEVE specific keys from the request
		if (specificKeys != null && specificKeys.size() > 0) {
	...
	public long getVariableAsLong(String key) {
		String value = getVariable(key);
		return !TextUtil.isEmpty(value) ? Long.parseLong(value) : 0;
	}
