...
<%@page import="gov.utcourts.corisweb.report.TrackingReportSearchCriteria"%>
<%@page import="gov.utcourts.corisweb.servlet.CorisReportTrackingServlet"%>
<%
	...
	String trackCode = (String) request.getAttribute("trackCode");
	Date dateFrom = (Date) request.getAttribute("dateFrom");
	Date dateTo = (Date) request.getAttribute("dateTo");
	
	StringBuilder trackingReasons = new StringBuilder();
	List<TrackingTypeBO> trackingTypeList = (List<TrackingTypeBO>) request.getAttribute("trackingReasons");
	if (trackingTypeList != null){
		trackingReasons.append("<option value=\"\">All</option>");
		for (TrackingTypeBO trackingType : trackingTypeList) {
			trackingReasons.append("<option value=\"")
			.append(trackingType.getTrackCode())
			.append(trackingType.getTrackCode().equals(trackCode) ? "selected" : "")
			.append("\">")
			.append(trackingType.getDescr())
			.append("</option>");
		}
	}
	String trackingReasonOptions = trackingReasons.toString();
	trackingReasons = null;
...
<head>
    ...
    <title>CORIS - Tracking Report</title>
...
<body>
				...
				<div class="card-body">
					...
					<form id="searchForm" name="searchForm">
						...
						<div class="form-row">
							<div class="form-group col-md-12 col-lg-12">
								<div class="row align-items-center">
									<label for="fieldName" class="col-sm-8 col-md-9 control-label text-sm-right"><strong>Tracking Reason</strong></label>
									<div class="col-sm-11 col-md-12">
										<select class="form-control" id="trackCode" name="trackCode"><%= trackingReasonOptions %></select>
									</div>
								</div>
							</div>
	...
	<script>
		...
		$(document).ready(function() {
			...
			$('#reportResultsDiv').load('<%= CorisReportTrackingServlet.RESULTS_PAGE %>');
		...
		function changeJudgeComm() {
			...
			appUX.ajax.call("/CorisWEB/CorisReportTrackingServlet",
		...
		function search() {
			if ($('#dateFrom').val() != '' && $('#dateTo').val() != '') {
				...
				$('#reportResultsDiv').load('/CorisWEB/CorisReportTrackingServlet?mode=find&orderBy=' + JSON.stringify(orderBy) + '&' + $('#searchForm').serialize(), 
		...
		function saveAs(format) {
			...
			window.open("/CorisWEB/CorisReportTrackingServlet?mode=save&format=" + format + "&orderBy=" + JSON.stringify(orderBy) + '&' + $('#searchForm').serialize());
		}
