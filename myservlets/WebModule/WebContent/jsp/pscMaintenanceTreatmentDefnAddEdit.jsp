...
String treatDefnId = TextUtil.getParamAsString(request, "treatDefnId");
...
<body>
	<div class="container-fluid">
		<div class="card m-2">
			<div class="card-header">
				<strong>
				<% if("add".equals(mode)) { %>
					Add New Treatment Definition
				<% } else if("edit".equals(mode)) { %>
					Edit Treatment Code: <%=code %>
				<% } %>
				</strong>
			</div>
			<div class="card-body">
				<form action="#" method="post">
					<input type="hidden" id="treatDefnId" name="treatDefnId" value="<%= treatDefnId %>">
					<div class="row">						
						<div class="input-group m-4">
							<label for="code" style="width: 33%; text-align: right; padding-right: 3px;">Treatment Code <span class="text-danger">*</span></label>
							...
							<div class="invalid-feedback" style="margin:0; padding:0; padding-bottom: 2px;"><span class="float-right" style="width: 66%;">Treatment Code is required and must be 8 or less letters, no spaces.</span></div>
						</div>
					</div>
					<div class="row">						
						<div class="input-group m-4">
							<label for="descr" style="width: 33%; text-align: right; padding-right: 3px;">Treatment Description <span class="text-danger">*</span></label>
							<input type="text" class="form-control" id="descr" name="descr" placeholder="Enter description" <%="edit".equalsIgnoreCase(mode)?"value='"+descr+"'":"" %> maxlength="40" required>
							<div class="invalid-feedback" style="margin:0; padding:0; padding-bottom: 2px;"><span class="float-right" style="width: 66%;">Treatment Description is required and must be 30 or less letters or spaces.</span></div>
						</div>
					</div>
					<div class="row">						
						<div class="input-group m-4">
							...
							<select class="form-control" id="courtType" name="courtType" required>
								...
								if(courtTypeList != null && courtTypeList.size() > 0) {
								 ...
								 %>
							 	<option value="<%= courtTypeBO.getPsCourtDefnId() %>"<%= TextUtil.isSelected(courtTypeBO.getPsCourtDefnId()+"", courtType)%>><%=psCourtType %></option>
							 <%
							 }
							 psCourtType = null;
							}
					...
					<div class="std-fixed bottom bg-light popcorn-ftr active">
						...
						<% if("add".equals(mode)) { %>
							<button type="button" class="btn btn-primary ml-2 mt-2 mr-2 btn-sm float-right" title="Save"onclick="saveForm($('#code').val(), $('#descr').val(), $('#courtType').val());">Save</button>
						<% } else if("edit".equals(mode)) { %>
							<button type="button" class="btn btn-primary ml-2 mt-2 mr-2 btn-sm float-right" title="Save" onclick="updateForm($('#code').val(), $('#descr').val(), $('#courtType').val(), $('#treatDefnId').val());">Save</button>
					<% } %>
				</div>
			</form>
		</div>
	...
	<script>
		...
		function updateForm(code, descr, courtType, treatDefnId) {
			...
			if(validatedDesc){
				appUX.ajax.call("/CorisWEB/PSCMaintenanceTreatmentDefnServlet", 
					function (err, data, xhr) { 
						appUX.pop.refreshCornFrame('modalTreatmentDefinitionsPrimary', false);
						closePop();
					}, 
					'POST', 
					'code='+code+'&courtId='+courtType+'&treatDefnId='+treatDefnId+'&descr='+descr+'&mode=edit&displayResults=true&displayDisabled=<%=displayDisabled%>'
				);
			}else{
		...
		function saveForm(code, descr, courtType) {
			...
			res = cleanCharacters(document.getElementById("descr")); //removes invalid characters
			if(!res) { validatedDesc = false; } //must be valid characters
			res = cleanCharacters(document.getElementById("code")); //removes invalid characters
			...
			if(validatedDesc && validatedCode){
				appUX.ajax.call("/CorisWEB/PSCMaintenanceTreatmentDefnServlet", 
					...
					'POST', 
					'code='+code+'&courtId='+courtType+'&descr='+descr+'&mode=add&displayResults=true&displayDisabled=<%=displayDisabled%>'
				);
			}else {
				if(!validatedDesc){
					//highlight the field in red and display message
