...
<%@page import="gov.utcourts.coriscommon.dataaccess.jdmttype.JdmtTypeBO"%>
...
List<JdmtTypeBO> jdmtTypeListBO = (List<JdmtTypeBO>) request.getAttribute("jdmtTypeListBO");
List<CaseTypeBO> caseTypeListBO = (List<CaseTypeBO>) request.getAttribute("caseTypeListBO");
...
<head>
    ...
    <title>CORIS - Charge Disposition Report</title>
...
<body>
    ...
    <main>
					...
					<form id="searchForm" name="searchForm" onsubmit="searchForm(event, false);">
						...
						<div id="warrantcontent" class= "row">
							<div class="col-md-6">
								<div class="form-group">
									<div class="row align-items-center">
										<label class="col-sm-8 col-md-9 control-label text-sm-right"><strong>Disposition<span class="text-danger">*</span></strong></label>
										<div class="col-sm-11 col-md-12">
											<div class="row">
												<div class="col input-group">
													<select class="form-control" id="jdmtType" name="jdmtType" required>
													</select>
												</div>
											</div>
										</div>
							...
							<div class="col-md-6">
								<div class="form-group">
									<div class="row align-items-center">
										<label class="col-sm-6 col-md-7 control-label text-sm-right"><strong>Case Type<span class="text-danger">*</span></strong></label>
							...
							<div class="col-md-12">
								<div class="form-group">
									<div class="row align-items-center">
										<label id="daterangeid" name="dateRange" class="col-sm-9 col-md-10 control-label text-sm-right"><strong>Disposition Date Start/End<span class="text-danger">*</span></strong></label>
    	...
    	<div id="jdmtTypeOptions" style="display: none;">
    		<%if (jdmtTypeListBO.size() > 0) {%>
				 <option value=""></option>
				<%for (JdmtTypeBO jdmtTypeBO : jdmtTypeListBO) {%>
					 <option value="<%=TextUtil.print(jdmtTypeBO.getJdmtCode())%>"><%=TextUtil.print(jdmtTypeBO.getDescr())%></option>
				<%}%>
			<%}%>
    	</div>
    	
    	<div id="caseTypeOptions" style="display: none;">
    		<%if (caseTypeListBO.size() > 0) {%>
				 <option value=""></option>
				<%for (CaseTypeBO caseTypeBO : caseTypeListBO) {%>
					 <option value="<%=TextUtil.print(caseTypeBO.getCaseType())%>"><%=TextUtil.print(caseTypeBO.getDescr())%></option>
				<%}%>
			<%}%>
    	</div>
    </main>
	...
	<script>
		...
		function clearForm(){
			$('#startDateId').val("");
			$('#endDateId').val("");
			$('#filingId').prop( "checked", false );
			$('#dispositionTypeId').prop( "checked", false );
		    changeDateText();
		}
		...
		function search() {
			if ($('#startDate').val() != '' && $('#endDate').val() != '') {
				...
				$('#reportResultsDiv').load('/CorisWEB/CorisReportChargeDispositionServlet?mode=find&orderBy=' + JSON.stringify(orderBy) + '&' + $('#searchForm').serialize(), 
	    ...
	    function loadJdmtType(){
			$('#jdmtType').html($('#jdmtTypeOptions').html());
	    }
	    
	    function loadCaseType(){
			$('#caseType').html($('#caseTypeOptions').html());
	    }
	    
		function saveReport(format){
			var table = $('#searchResultsTable').DataTable();
			var orderBy = table.order();
			windowOpenUrlEncrypt("/CorisWEB/CorisReportChargeDispositionServlet", "mode=save&orderBy=" + JSON.stringify(orderBy) + "&format="+format+"&"+$('#searchForm').serialize());
		}
		...
		function emailReport(format){
			...
			var url = '/CorisWEB/CorisReportChargeDispositionServlet?mode=email&locnCode=<%=locnCode%>&courtType=<%=courtType%>&format=' + format;
			appUX.ajax.call(url, 
