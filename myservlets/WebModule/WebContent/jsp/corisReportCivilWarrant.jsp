		...
		<div class="container-fluid">
			...
			<div class="card m-2">
				...
				<div class="card-header">
					<strong>Civil Warrant Report</strong>
				</div>
	...
	<script>
		...
		function search() {
			if ($('#startDate').val() != '' && $('#endDate').val() != '') {
				...
				$('#reportResultsDiv').load('/CorisWEB/CorisReportCivilWarrantServlet?mode=find&orderBy=' + JSON.stringify(orderBy) + '&' + $('#searchForm').serialize(), 
		...
		function saveReport(format){
			var table = $('#searchResultsTable').DataTable();
			var orderBy = table.order();
			windowOpenUrlEncrypt("/CorisWEB/CorisReportCivilWarrantServlet", "mode=save&orderBy=" + JSON.stringify(orderBy) + "&format="+format+"&"+$('#searchForm').serialize());
		}
		
		function emailReport(format){
			var url = '/CorisWEB/CorisReportCivilWarrantServlet?mode=email&locnCode=<%=locnCode%>&courtType=<%=courtType%>&format=' + format;
			appUX.ajax.call(url, 
