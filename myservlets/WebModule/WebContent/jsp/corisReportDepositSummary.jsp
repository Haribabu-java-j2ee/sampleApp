...
<%@page import="gov.utcourts.corisweb.report.DepositSummaryReportSearchCriteria"%>
<%@page import="gov.utcourts.corisweb.servlet.CorisReportDepositSummaryServlet"%>
<%
	...
	Integer journalFrom = (Integer) request.getAttribute("journalFrom");
	Integer journalTo = (Integer) request.getAttribute("journalTo");
...
<head>
    ...
    <title>CORIS - Deposit Summary Report</title>
...
<body>
			...
			<div class="card m-2">
				...
				<div class="card-body">
					...
					<form id="searchForm" name="searchForm">
						<input type="hidden" id="dateOrJournal" name="dateOrJournal" value="byDate">
						<input type="hidden" id="summaryOrDetail" name="summaryOrDetail" value="summary">
						<div class="form-row">
							<div class="form-group col-md-8 col-lg-8">
								<div class="row align-items-center">
									...
									<div class="col-sm-11 col-md-12">
										<select class="form-control" id="courtType" name="courtType" onchange="loadLocation(this.value)">
							...
							<div class="form-group col-md-13 col-lg-10">
								...
								<div class="row align-items-center">
									<label for="summaryOrDetail1" class="radio-inline col control-label text-sm-center">
									<input type="radio" name="summaryOrDetail" id="summaryOrDetail1" value="summary" onclick="changeSummaryOrDetail('summary')" checked>&nbsp;Summary</label>
									<label for="summaryOrDetail2" class="radio-inline col control-label text-sm-center">
									<input type="radio" name="summaryOrDetail" id="summaryOrDetail2" value="detail" onclick="changeSummaryOrDetail('detail')">&nbsp;Detail</label>
								</div>
							</div>
							<div class="card-body">
								<ul class="nav nav-tabs">
									<li class="nav-item">
										<a href="#byDate" class="nav-link active" onclick="changeDateOrJournal('byDate');" data-toggle="tab">By Date</a>
									</li>
									<li class="nav-item">
										<a href="#byJournal" class="nav-link" onclick="changeDateOrJournal('byJournal');" data-toggle="tab">By Journal</a>
									</li>
								</ul>
								<div class="tab-content">
									...
									<div class="tab-pane fade" id="byJournal">
										<div class="form-group col-md-12 col-lg-12">
											<div class="row">&nbsp;</div>
											<div class="row align-items-center">
												<label for="fieldName" id="byJournalLabel" class="col-sm-8 col-md-9 control-label text-sm-right"><strong>From/To</strong></label>
												<div class="col-sm-11 col-md-12">
													<div class="row">
														<div class="col input-group">
															<input type="number" oninput="this.value = Math.abs(this.value);" class="form-control" id="journalFrom" name="journalFrom" value="<%= TextUtil.print(journalFrom) %>">
														</div>
														<div class="col input-group">
															<input type="number" oninput="this.value = Math.abs(this.value);" class="form-control" id="journalTo" name="journalTo" value="<%= TextUtil.print(journalTo) %>">
														</div>
													</div>
												</div>
											</div>
	...
	<script>
		...
		function changeDateOrJournal(value){
			$('#dateOrJournal').val(value);
		}
		
		function changeDateOrJournal(type) {
			$('#dateOrJournal').val(type);
		}
		
		function changeSummaryOrDetail(type) {
			$('#summaryOrDetail').val(type);
			
			if (type == 'summary') {
				$('#byJournalLabel').html('<strong>From/To<strong>');
				$('#journalTo').show();
			} else {
				$('#byJournalLabel').html('<strong>From<strong>');
				$('#journalTo').hide();
			}
		}
		...
		function isValid() {
			var validationError = '';
			if ($('#locnCode').val() == null) {
				validationError = "Location must be selected.";
			}
			
			if ($('#dateOrJournal').val() == 'byDate') {
				if ($('#dateFrom').val() == '' && $('#dateTo').val() == '')
					validationError = "From/To dates must be selected.";
				else if ($('#dateFrom').val() == '') 
					validationError = "From date must be selected.";
				else if ($('#dateTo').val() == '') 
					validationError = "To date must be selected.";
			} else {
				if ($('#summaryOrDetail').val() == 'summary') {
					if ($('#journalFrom').val() == '' && $('#journalTo').val() == '')
						validationError = "From/To journals must be selected.";
					else if ($('#journalFrom').val() == '')
						validationError = "From journal must be selected.";
					else if ($('#journalTo').val() == '')
						validationError = "To journal must be selected.";
				} else {
					if ($('#journalFrom').val() == '')
						validationError = "From journal must be selected.";
				}
			...
			if (validationError == '') {
				...
				appUX.pop.declare("Deposit Summary Report", validationError, 300, 'auto', appUX.pop.styles.DANGER);
		...
		function saveAs(format) {
			if (isValid()) {
				windowOpenUrlEncrypt("/CorisWEB/CorisReportDepositSummaryServlet", "mode=save&format=" + format + '&' + $('#searchForm').serialize());
			}
		}
		
		function emailAs(format) {
			if (isValid()) {
				appUX.ajax.call("/CorisWEB/CorisReportDepositSummaryServlet",
					...
					'POST',
					'mode=email&format=' + format + '&' + $('#searchForm').serialize()
				);
			}
		}
