						...
						<div class="col-12">
							<div class="form-group col-md-16 offset-lg-2 col-lg-18">
								<div class="row align-items-center">
									...
									<div class="col-sm-12 col-md-13">
										<input type="checkbox" id="fileDate" name="fileDate" checked /><label for="fileDate">Filing Date</label>
									</div>
								</div>
							</div>
						</div>
						...
						<div class="col-12">
							<div class="form-group col-md-16 offset-lg-2 col-lg-18">
								<div class="row align-items-center">
									<label class="col-sm-8 col-md-9 control-label text-sm-right"><strong><span class="text-danger"></span></strong></label>
									<div class="col-sm-12 col-md-13">
										<input type="checkbox" id="dispDate" name="dispDate" /><label for="dispDate">Disposition Date</label>
									</div>
								</div>
							</div>
						</div>
...
<!-- footer -->
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$('#reportResultsDiv').load('jsp/corisReportDomViolResults.jsp');
		...
		function search() {
			...
			if(validateReportDate()){
				...
				var url = '/CorisWEB/CorisReportDomViolServlet?mode=find&' + $('#searchForm').serialize() 
		...
		function saveReport(fmt){
			...
			if(validateReportDate()){
				var dataTable = $('#searchResultsTable').DataTable();
				var order = dataTable.order();
				window.open("/CorisWEB/CorisReportDomViolServlet?mode=save&locnCode=<%=criteria.getLocnCode()%>&courtType=<%=criteria.getCourtType()%>&format=" + fmt 
		...
		function emailReport(fmt){
			...
			if(validateReportDate()){
				var dataTable = $('#searchResultsTable').DataTable();
				var order = dataTable.order();
				var url = '/CorisWEB/CorisReportDomViolServlet?mode=email&locnCode=<%=criteria.getLocnCode()%>&courtType=<%=criteria.getCourtType()%>&format=' + fmt
